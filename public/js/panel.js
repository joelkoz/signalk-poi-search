(() => {
  ...
  function renderResults() {
    ...
    list.innerHTML = lastResults.slice(0, 50).map(
      ([id, n]) => `<li><span class="poi-name">${esc(n.name ?? id)}</span>
  ...
  async function main() {
    ...
    <label class="row"><span>Category</span>
      ...
      ([value, title]) => `<option value="${value}" ${saved.category === value ? "selected" : ""}>${title}</option>`
    ).join("")}</select></label>
